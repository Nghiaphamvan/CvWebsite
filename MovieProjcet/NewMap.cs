...
namespace MovieProjcet
{
    ...
    "y","y","y","y","y",};
        ...
        {
            string ress = RemoveUnicode(Address);

            StringBuilder queryaddress = new StringBuilder();
            queryaddress.Append("https://www.google.com/maps/dir/KTX+Khu+B,+ĐHQG,+To+Vinh+Dien,+Dong+Hoa,+Di+An,+Binh+Duong/");

            List<String> listStrLineElements = ress.Split(',').ToList();
            foreach (string item in listStrLineElements)
            {
                queryaddress.Append(item + "," + "+");
            }
