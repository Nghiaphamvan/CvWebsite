...
{
    public partial class Toronto_CV : Form
    {
        Bitmap MemoryImage;
                ...
                if (newform.ShowDialog() == DialogResult.OK)
                {
                    if (newform.format == "PNG")
                    {
                        try
                        {
                            using (var bmp = new Bitmap(pnl_CV.Width, pnl_CV.Height))
                            {
                                ...
                                bmp.Save(@"C:\Users\HP\Desktop\YourResume.png");
                            }
                    ...
                    else if (newform.format == "JPG")
                    {
                        try
                        {
                            ...
                            {
                                pnl_CV.DrawToBitmap(bmp, new Rectangle(0, 0, bmp.Width, bmp.Height));
                                bmp.Save(@"C:\Users\HP\Desktop\YourResume.jpg");
                            }
                            MessageBox.Show("CV saved successfully.");
                        }
                        catch
        ...
        {
            lbl_WantedJobTitle.Text = txt_WantedJobTitle.Text;

            lbl_Email.Text = txt_Email.Text;

            lbl_FirstName.Text = txt_FirstName.Text;
            ...
            if (txt_Address.Text != "")
            {
                ...
                if (txt_City.Text != "")
                {
                    lbl_Address.Text = lbl_Address.Text + ", " + txt_City.Text;
                    if (txt_Country.Text != "")
                    {
                        lbl_Address.Text = lbl_Address.Text + ", " + txt_Country.Text;
                    }
                }
            ...
            {
                lbl_Address.Text += txt_City.Text;
                if (txt_Country.Text != "")
                {
                    lbl_Address.Text = lbl_Address.Text + ", " + txt_Country.Text;
                }
            ...
            {
                lbl_Address.Text += txt_Country.Text;
            }

            lbl_DateandPlaceofBirth.Text = "";
            lbl_TitleDateAndPlaceOfBirth.Text = "";
            if (txt_DateOfBirth.Text != "")
            {
                lbl_DateandPlaceofBirth.Text += txt_DateOfBirth.Text;
                lbl_TitleDateAndPlaceOfBirth.Text = "Date of birth";
                if (txt_PlaceOfBirth.Text != "")
                {
                    lbl_DateandPlaceofBirth.Text = lbl_DateandPlaceofBirth.Text + " / " + txt_PlaceOfBirth.Text;
                    lbl_TitleDateAndPlaceOfBirth.Text = "Date / Place of birth";
                }
            }
            else if (txt_PlaceOfBirth.Text != "")
            {
                lbl_DateandPlaceofBirth.Text += txt_PlaceOfBirth.Text;
                lbl_TitleDateAndPlaceOfBirth.Text = "Place of birth";
            }

            lbl_ProfessionalSummary.Text = txt_ProfessionalSummary.Text;

            lbl_Nationality.Text = txt_Nationality.Text;
            if (lbl_Nationality.Text == "")
            {
                lbl_TitleNationality.Text = "";
            }

            Flpnl_EmploymentHistory.Controls.Clear();
            foreach (Attr_Employment item in Flpnl_Employment_Body.Controls)
            {
                Toronto_Employment newEmployment = new Toronto_Employment();
            ...
            {
                Toronto_Education newEducation = new Toronto_Education();
            ...
            {
                Toronto_Skill newSkill = new Toronto_Skill();
        ...
        private void btn_Add_Employment_Click_1(object sender, EventArgs e)
        {
            Attr_Employment newEmployment = new Attr_Employment();
            Flpnl_Employment_Body.Controls.Add(newEmployment);
        }
